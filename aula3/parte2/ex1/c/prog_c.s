...
main:   addiu $sp,$sp,-12                    # Reservar espaço na stack
        ...
        sw $s0,4($sp)                       # Guardar $s1 na stack
...
while:  li $a0,500                          #while(True){
        ...
        lw $t1,PORTB($s0)                   # Read PORTB
        andi $t1,$t1,0x0008                 #
        srl $t1,$t1,3                       # St1 = PortB_bit3

inc:    bne $t1,1,dec                       # if($t1 == 1){
        addiu $s1,$s1,1                     #   counter++;
        j end                               # }
dec:    addiu $s1,$s1,-1                    # else{counter--;
end:    j while                             #}
