...
if_c:   bne $s2,'+',else_c1       #     if( c == '+' )
        li $s0,0                  #         s = 0; 
        j skip

else_c1: bne $s2,'-',else_c2       #     if( c == '-' )
         li $s0,1                  #         s = 1; 
         j skip                    #

else_c2: bne $s2,'s',else_c3       #     if( c == '-' )
         li $s0,3                  #         s = 1; 
         j skip                    #

else_c3: bne $s2,'r',skip          #     if( c == '-' )
         li $s0,2                  #         s = 1; 
         j skip                    #
...
if_s:  bnez $s0,else_s1            #    if( s == 0 ) 
       addi $s1,$s1,1             #         cnt = cnt + 1;
       andi $s1,$s1,0XFF          #         cnt = cnt & 0xFF; 
       j while                    #

else_s1: bne $s0,1,else_s2         #   else if( s == 1 )
        addi $s1,$s1,-1            #         cnt = cnt - 1;
        andi $s1,$s1,0XFF          #         cnt = cnt & 0xFF;
        j while                    #
